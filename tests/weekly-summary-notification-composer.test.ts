...
describe("WeeklySummaryNotificationComposer", async () => {
  ...
  test("compose - en", () => {
    expect(en.compose(mocks.weeklySummary)).toEqual({
      ...
      html: v.parse(
        ...
        Notifications.Services.WeeklySummaryLayout.render({
  ...
  test("compose - no workouts", () => {
    ...
    expect(notification.html).toContain("No workouts completed this week.");
    expect(notification.html).not.toContain("vs last week");
    expect(notification.html).not.toContain("Progress");
  });
  ...
  test("compose - body weight on track for cut", () => {
    ...
    expect(notification.html).toContain("80.5 kg");
    expect(notification.html).toContain("average, 4 measurements");
    expect(notification.html).toContain("-0.3 kg vs last week · on track for cut");
  });
  ...
  test("compose - body weight drifting from bulk", () => {
    ...
    expect(notification.html).toContain("average, 1 measurement<");
    expect(notification.html).toContain("-0.3 kg vs last week · drifting from bulk");
  });
  ...
  test("compose - body weight unchanged, maintain goal", () => {
    ...
    expect(notification.html).toContain("unchanged vs last week");
  });
  ...
  test("compose - body weight without previous week or goal", () => {
    ...
    expect(notification.html).toContain("80.5 kg");
    expect(notification.html).not.toContain("kg vs last week ·");
    expect(notification.html).not.toContain("unchanged");
  });
  ...
  test("compose - polish plural forms", () => {
    ...
    expect(notification.html).toContain(">treningów<");
    expect(notification.html).toContain("średnio, 3 pomiary");
    expect(notification.html).toContain("1 trening ukończony w tym miesiącu");
  });

  test("compose - escapes user content", () => {
    const notification = en.compose({
      ...mocks.weeklySummary,
      highlights: [
        {
          ...mocks.weeklySummaryHighlight,
          exerciseName: v.parse(Exercises.VO.ExerciseName, "<b>Bench</b> & co"),
        },
      ],
    });

    expect(notification.html).toContain("&lt;b&gt;Bench&lt;/b&gt; &amp; co");
    expect(notification.html).not.toContain("<b>Bench</b>");
  });
});
