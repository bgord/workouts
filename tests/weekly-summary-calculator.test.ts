...
describe("WeeklySummaryCalculator", () => {
  ...
  test("numbers - delta against previous week", () => {
    ...
    expect(calculator.calculate()?.sections).toEqual([
      {
  ...
  test("highlights - reps up at lower load is not progress", () => {
    ...
    expect(calculator.calculate()?.sections).toEqual([mocks.weeklySummaryNumbersSection]);
  });
  ...
  test("body weight - only section", () => {
    ...
    expect(calculator.calculate()?.sections).toEqual([
      mocks.weeklySummaryEmptyNumbersSection,
      {
