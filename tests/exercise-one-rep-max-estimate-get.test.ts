...
const url = `/api/statistics/exercises/${mocks.exerciseId}/one-rep-max-estimate`;
...
describe(`GET ${url}`, async () => {
  ...
  test("happy path", async () => {
    ...
    expect(json).toEqual({
      oneRepMax: {
        set: { id: mocks.loggedSetId, workoutId: mocks.workoutId, reps: 5, load: 90000 },
        estimate: 105000,
      },
    });
  });
  ...
  test("happy path - no sets logged", async () => {
    const spies = new DisposableStack();
    spies.use(spyOn(di.Tools.Auth.config.api, "getSession").mockResolvedValue(mocks.auth));
    spies.use(spyOn(di.Adapters.Workouts.ListExerciseSetsQuery, "execute").mockResolvedValue([]));

    const response = await server.request(url, { method: "GET" }, mocks.ip);
    const json = await response.json();

    expect(response.status).toEqual(200);
    expect(json).toEqual({ oneRepMax: null });
  });
});
