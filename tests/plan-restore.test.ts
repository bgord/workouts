...
const url = `/api/plans/${mocks.planId}/restore`;

describe("POST /api/plans/:planId/restore", async () => {
  ...
  test("PlanIsRestorable - initial", async () => {
    ...
    await testcases.assertInvariantError(response, 403, "plan.is.restorable");
  ...
  test("happy path", async () => {
    ...
    expect(eventStoreSave).toHaveBeenCalledWith([mocks.GenericPlanRestoredEvent]);
  });
});
