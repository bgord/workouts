...
const url = "/api/measurements/body-weight/export";
...
describe(`GET ${url}`, async () => {
  ...
  test("happy path", async () => {
    ...
    expect(response.headers.get("content-disposition")).toEqual(
      `attachment; filename="body-weight-measurement-export-${mocks.T0.ms}.csv"`,
    );
    expect(await response.text()).toEqualIgnoringWhitespace(mocks.bodyWeightMeasurementCsv);
  });
});
