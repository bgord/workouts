...
const url = `/api/workouts/${mocks.workoutId}/exercise/${mocks.workoutExerciseId}/target`;

describe("PATCH /api/workouts/:workoutId/exercise/:workoutExerciseId/target", async () => {
  ...
  test("validation - incorrect workout id", async () => {
    ...
    const response = await server.request(
      `/api/workouts/id/exercise/${mocks.workoutExerciseId}/target`,
  ...
  test("validation - incorrect workout exercise id", async () => {
    ...
    const response = await server.request(
      `/api/workouts/${mocks.workoutId}/exercise/id/target`,
      { method: "PATCH", body: JSON.stringify({}) },
      mocks.ip,
    );
    const json = await response.json();
  ...
  test("validation - reps - missing", async () => {
    ...
    const response = await server.request(
      url,
      {
        ...
        body: JSON.stringify({ sets: mocks.exerciseTarget.sets }),
      },
      mocks.ip,
    );
    const json = await response.json();
  ...
  test("validation - load - missing", async () => {
    ...
    const response = await server.request(
      url,
      {
        method: "PATCH",
        headers: mocks.revisionHeaders(),
        body: JSON.stringify({ sets: mocks.exerciseTarget.sets, reps: mocks.exerciseTarget.reps }),
      },
      mocks.ip,
    );
    const json = await response.json();
  ...
  test("validation - load - invalid", async () => {
    ...
    const response = await server.request(
      url,
      {
        method: "PATCH",
        headers: mocks.revisionHeaders(),
        body: JSON.stringify({ ...mocks.exerciseTarget, load: -1 }),
      },
      mocks.ip,
    );
    const json = await response.json();
  ...
  test("revision mismatch", async () => {
    ...
    const response = await server.request(
      url,
      { method: "PATCH", headers: mocks.revisionHeaders(99), body: JSON.stringify(mocks.exerciseTarget) },
      mocks.ip,
    );
