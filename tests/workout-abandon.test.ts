...
const url = `/api/workouts/${mocks.workoutId}/abandon`;

describe("PATCH /api/workouts/:workoutId/abandon", async () => {
  ...
  test("validation - incorrect workout id", async () => {
    ...
    const response = await server.request("/api/workouts/id/abandon", { method: "PATCH" }, mocks.ip);
    const json = await response.json();
  ...
  test("happy path", async () => {
    ...
    expect(eventStoreSave).toHaveBeenCalledWith([mocks.GenericWorkoutAbandonedEvent]);
  });
});
