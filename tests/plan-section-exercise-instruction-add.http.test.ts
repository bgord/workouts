...
describe("POST /api/plans/:planId/section/:planSectionId/exercise-instruction", async () => {
  ...
  test("PlanSectionExerciseInstructionLimit", async () => {
    ...
    await testcases.assertErrorResponse(response, 403, "plan.section.exercise.instruction.limit");
    expect(eventStoreSave).not.toHaveBeenCalled();
  });
