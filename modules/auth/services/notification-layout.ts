...
export class NotificationLayout {
  static render(config: NotificationLayoutConfig): string {
    return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<tr><td align="center" style="padding:40px 16px;">
  ...
  <tr><td style="padding:28px 32px;font-family:system-ui,-apple-system,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;">
    ...
    <p style="margin:12px 0 0;font-size:15px;line-height:1.6;color:#3c4047;">${config.intro}</p>
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width:auto;margin-top:24px;"><tr>
      <td style="background:#0f131a;border-radius:8px;"><a href="${config.url}" style="display:inline-block;padding:0 16px;height:36px;line-height:36px;font-size:14px;font-weight:500;color:#fcfcfc;text-decoration:none;">${config.cta}</a></td>
    </tr></table>
    <p style="margin:24px 0 0;font-size:13px;line-height:1.6;color:#55585e;">${config.note}</p>
    <p style="margin:12px 0 0;font-size:13px;line-height:1.6;color:#55585e;">Button not working? Paste this into your browser:<br /><a href="${config.url}" style="color:#ca5a15;text-decoration:underline;word-break:break-all;">${config.url}</a></p>
