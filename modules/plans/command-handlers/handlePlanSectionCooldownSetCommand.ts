...
export const handlePlanSectionCooldownSetCommand =
  (deps: Dependencies) => async (command: Plans.Commands.PlanSectionCooldownSetCommandType) => {
    const plan = await deps.repo.load(command.payload.planId);
    command.revision.validate(plan.revision.value);
    plan.setSectionCooldown(
      command.payload.planSectionId,
      command.payload.cooldown,
