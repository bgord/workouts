...
export const handleWorkoutCompleteCommand =
  (deps: Dependencies) => async (command: Workouts.Commands.WorkoutCompleteCommandType) => {
    const workout = await deps.repo.load(command.payload.workoutId);
    command.revision.validate(workout.revision.value);
    workout.complete(command.payload.requesterId);
