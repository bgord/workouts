...
export function createServer({ Env, Adapters, Tools }: BootstrapType) {
  ...
  plans.patch(
    "/:planId/section/:planSectionId/cooldown",
    Tools.ShieldCaptcha.handle(),
    bg.EndpointHonoAdapter.adapt(HTTP.Plans.PlanSectionCooldownSet(deps)),
  );
  plans.delete(
  ...
  workouts.patch(
    "/:workoutId/exercise/:workoutExerciseId/position",
    Tools.ShieldCaptcha.handle(),
    bg.EndpointHonoAdapter.adapt(HTTP.Workouts.WorkoutExerciseMove(deps)),
  );
  workouts.delete(
  ...
  measurements.query(
    "/body-weight/chart",
    bg.EndpointHonoAdapter.adapt(HTTP.Measurements.BodyWeightChartGet(Adapters.Measurements)),
  );
  measurements.get(
    "/body-weight/export",
    Tools.ShieldRateLimit.handle(),
